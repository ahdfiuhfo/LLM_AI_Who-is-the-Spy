...
true_player = input(f"你是否希望参与游戏，如果想，请输入以下列表任一元素的内容，否则输入任意内容: \n{now_play_list}")
...
while len(now_play_list)-len(wodi_players) > len(wodi_players)  :
    ...
    for name in now_play_list:#循环发言！
        if name == true_player:
            print(f"你是{name}，请发言。你的词语是{player_roles[name]}。")
            fayan[name] = input("输入:")
        else:
    ...
    for name in now_play_list:
        if name == true_player:
            vote = input(f"请投票，以下是所有的玩家的发言：{fayan}。请根据所有人的发言选择你想投票的人：{now_play_list}。")
